...
`hermione-тесты` — это [интеграционные тесты](http://www.intuit.ru/studies/courses/1040/209/lecture/5412), которые разрабатываются и запускаются в браузере с помощью утилиты [Hermione](https://github.com/gemini-testing/hermione). Утилита основана на  [WebdriverIO](http://webdriver.io/) и [Mocha](https://mochajs.org) с рядом дополнительных возможностей, упрощающих тестирование: быстрое начало разработки с использованием уже знакомых инструментов, параллельный запуск тестов, повторное выполнение нестабильных тестов при неудачном завершении и т. п.

Основной задачей интеграционного тестирования является проверка правильности взаимодействия отдельных компонентов сервиса путем эмуляции действий пользователя в браузере, которые определяют функциональность приложения.
...
Как правило, для реализации тестов необходимо несколько составляющих:

- набор тестовых сценариев, которые копируют сценарии использования приложения; 
- набор тестовых данных (дамп), конкретные значения на основе которых оценивается правильность выполнения тестов.
...
Чтобы разрабатывать и выполнять тесты, помиомо утилиты `Hermione`, понадобятся следующие програмнные средства:

* [NodeJS](https://nodejs.org/en/) не ниже 4 версии.
* [Selenium](https://github.com/vvo/selenium-standalone/blob/master/README.md) — инструмент, необходимый для разработки тестов с использованием [WebdriverIO](http://webdriver.io/) и запуска тестов в локальном [Selenium Grid](https://wiki.yandex-team.ru/selenium/).
...
Настраивается утилита в файле `.hermione.conf.js`, расположенном в корне проекта.
